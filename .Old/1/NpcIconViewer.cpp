/**
 * NpcIconViewer – display NpcIconsPacked.bin
 * ============================================
 * Loads GameData/Icons/NpcIconsPacked.bin (square atlas, 9‑bit colour),
 * scales each 3‑bit channel back to 8‑bit (×36),
 * flips the image vertically to match OpenGL,
 * and displays the atlas full‑screen with pan & zoom.
 *
 * Compile (must use UCRT64 shell):
 *   g++ -std=c++20 -O2 -static -DGLEW_STATIC \
 *     -o NpcIconViewer.exe NpcIconViewer.cpp \
...
int main()
{
    const std::string binPath = "GameData/Icons/NpcIconsPacked.bin";
    ...
    // Determine atlas side (must be square, 2 bytes per pixel)
    int texSide = (int)std::sqrt(fileSize / sizeof(u16));
    if (texSide * texSide * sizeof(u16) != fileSize)
    {
        std::cerr << "File size not a perfect square\n";
        return 2;
    }
    std::cout << "Atlas size: " << texSide << "x" << texSide << "\n";

    std::vector<u16> packed(texSide * texSide);
    file.read(reinterpret_cast<char *>(packed.data()), fileSize);

    // Convert 9‑bit colour to RGBA (scale 0‑7 → 0‑255 by *36)
    std::vector<u8> rgba(texSide * texSide * 4);
    ...
    for (int row = 0; row < texSide / 2; ++row)
    {
        int topRow = row;
        int bottomRow = texSide - 1 - row;
        for (int col = 0; col < texSide * 4; ++col)
        {
            std::swap(rgba[topRow * texSide * 4 + col],
                      rgba[bottomRow * texSide * 4 + col]);
        }
    }
    ...
    GLFWwindow *win = glfwCreateWindow(mode->width, mode->height, "NPC Icon Viewer", mon, nullptr);
    if (!win)
        return 4;
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, texSide, texSide, 0, GL_RGBA,
